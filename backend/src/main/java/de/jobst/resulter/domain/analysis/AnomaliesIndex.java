...
/**
 * Represents the result of the anomaly detection analysis for a single segment.
 */
public record AnomaliesIndex(
    ...
    double cleanedReferenceTime) { // The actual reference time used for classification
    ...
    public static AnomaliesIndex of(int legNumber,
                                    ...
                                    AnomalyClassification classification,
                                    double cleanedReferenceTime) {
        double aiValue = piReal.value() / piExpected.value();
        return new AnomaliesIndex(legNumber, segmentKey, piReal, piExpected, aiValue, classification, cleanedReferenceTime);
    }
}
