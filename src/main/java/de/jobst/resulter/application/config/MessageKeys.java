package de.jobst.resulter.application.config;

public class MessageKeys {
    public static final MessageKey RESOURCE_NOT_FOUND = new MessageKey("resource_not_found");
    public static final MessageKey NO_RESOURCE_FOUND = new MessageKey("no_resource_found");
    public static final MessageKey RESPONSE_NOT_FOUND = new MessageKey("response_not_found");
    public static final MessageKey DATA_INTEGRITY_VIOLATION = new MessageKey("data_integrity_violation");
    public static final MessageKey ILLEGAL_ARGUMENT = new MessageKey("illegal_argument");
    public static final MessageKey UNEXPECTED_ERROR = new MessageKey("unexpected_error");
    public static final MessageKey SUCCESSFULLY_RETRIEVED = new MessageKey("successfully_retrieved");
    public static final MessageKey BAD_REQUEST = new MessageKey("bad_request");
    public static final MessageKey ENTITY_LOCK_CONFLICT = new MessageKey("entity_lock_conflict");
}
