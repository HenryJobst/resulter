...
public class ClassResult implements Comparable<ClassResult> {
    ...
    public static ClassResult of(long eventId,
                                 ...
                                 @NonNull Gender gender,
                                 @Nullable Collection<PersonResult> personResults) {
        return ClassResult.of(ClassResultId.empty().value(), eventId, name, shortName, gender, personResults);
    }
    ...
    public static ClassResult of(long id,
                                 ...
                                 @NonNull Gender gender,
                                 @Nullable Collection<PersonResult> personResults) {
        return new ClassResult(ClassResultId.of(id),
            EventId.of(eventId),
    ...
    public void calculate(CupTypeCalculationStrategy cupTypeCalculationStrategy) {
        Set<RaceNumber> races = this.getPersonResults()
            .value()
            .stream()
            .flatMap(it -> it.getPersonRaceResults().value().stream())
            .map(PersonRaceResult::getRaceNumber)
            .collect(Collectors.toSet());
        races.forEach(raceNumber -> {
            List<PersonResult> personResults =
                this.getPersonResults().value().stream().filter(cupTypeCalculationStrategy::valid).sorted().toList();
