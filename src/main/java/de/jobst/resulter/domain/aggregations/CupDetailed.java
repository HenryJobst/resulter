...
public class CupDetailed extends Cup {
    ...
    private void initializeGroupedByOrganisationData() {
        ...
        this.overallOrganisationScores = organisationWithScore.entrySet().stream()
                .map(entry -> {
                    Map<ClassResultShortName, List<ClassResultShortNameScoreSummary>> groupedScores =
                            organisationScores.stream()
                                    ...
                                    .collect(Collectors.toMap(
                                            ...
                                            x -> new ArrayList<>(
                                                    List.of(new ClassResultShortNameScoreSummary(x.score(), x.id()))),
                                            (existing, merging) -> {
                                                existing.addAll(merging);
                                                return existing;
                                            }));

                    Map<ClassResultShortName, List<ClassResultShortNameScoreSummary>> completeScores =
                            allClasses.stream()
                                    .collect(Collectors.toMap(
                                            ClassResultShortName::of,
                                            classResultShortName -> List.of(
                                                    new ClassResultShortNameScoreSummary(0.0, PersonId.empty()))));

                    completeScores.putAll(groupedScores);

                    List<PersonWithScore> personWithScores = completeScores.entrySet().stream()
